...
const TopSellingProduct = ({ setFieldValue, values }) => {
    ...
    const { data, refetch, isLoading } = useQuery({queryKey: [product, values['filter_by']], queryFn: () => request({ url: product, params: { status: 1, top_selling: 1, filter_by: values['filter_by']?.value, paginate: 5 } }), enabled: false, refetchOnWindowFocus: false, select: (data) => data.data.data });
