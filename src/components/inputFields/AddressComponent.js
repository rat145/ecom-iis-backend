...
const AddressComponent = ({ values, noAddress }) => {
  ...
  const { data, isLoading } = useQuery({queryKey: [country], queryFn: () => request({ url: country }), refetchOnWindowFocus: false, select: (res) => res.data.map((country) => ({ id: country.id, name: country.name, state: country.state })) });
