...
const Checkout = () => {
  ...
  const { data: addToCartData, isLoading: addToCartLoader, refetch } = useQuery({queryKey: [AddtoCartAPI], queryFn: () => request({ url: AddtoCartAPI }), refetchOnWindowFocus: false, cacheTime: 0, select: (res) => res?.data });
  // Getting Users data
  const { data: userData, refetch: userRefetch, isLoading: userLoader } = useQuery({queryKey: [user], queryFn: () => request({ url: user, params: { role: 'consumer', status: 1, paginate: 15, search: customSearch ? customSearch : "" } }), enabled: false, refetchOnWindowFocus: false, select: (data) => data.data.data });
