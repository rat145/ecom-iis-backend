...
const DeliveryAddress = ({ values, updateId, type, title }) => {
  ...
  const { data, isLoading: load, refetch } = useQuery({queryKey: [user, updateId], queryFn: () => request({ url: `${user}/${updateId}` }), enabled: false, refetchOnWindowFocus: false, select: (data) => (data.data) });
