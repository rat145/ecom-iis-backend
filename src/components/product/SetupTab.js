...
const SetupTab = ({ values, setFieldValue, errors, updateId }) => {
  ...
  const { data: categoryData } = useQuery({queryKey: [Category], queryFn: () => request({ url: Category, params: { type: "product" } }), refetchOnWindowFocus: false, select: (data) => data.data.data });

  // Getting Tags Data with type products
  const { data: tagData } = useQuery({queryKey: [tag], queryFn: () => request({ url: tag, params: { type: "product" } }), refetchOnWindowFocus: false, select: (data) => data.data.data });
  ...
  const { data: productData, isLoading: productLoader, refetch } = useQuery({queryKey: [product, arrayState], queryFn: () => request({
    url: product, params:
    {
      status: 1,
