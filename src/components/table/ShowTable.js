...
const ShowTable = ({ isCheck, setIsCheck, url, sortBy, setSortBy, headerData, fetchStatus, moduleName, type, redirectLink, refetch, keyInPermission }) => {
  ...
  const isHandleEdit = (e, tableData, headerData) => {
    e.preventDefault();
          ...
          headerData?.data?.map((tableData, index) => (
            <tr key={index}>
              ...
              {headerData.isSerialNo !== false && (
                <td className="sm-width" onClick={(e) => isHandleEdit(e, headerData, tableData)}>
                  {index + 1 + (1 - 1) * 30}
                </td>
              )}
              <>
                {headerData?.column.map((item, i) => (
                  <td className={item.type == "image" ? "sm-width" : ""} key={i} onClick={(e) => item.type !== "switch" && isHandleEdit(e, tableData, headerData)}>
                    {item.type == "date" ? (
