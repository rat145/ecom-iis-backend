...
const FooterTab = ({ values, setFieldValue, errors, categoryData }) => {
  ...
  const { data: pageData, isLoading: pageLoader } = useQuery({queryKey: [PagesAPI], queryFn: () => request({ url: PagesAPI }), refetchOnWindowFocus: false, select: (data) => data.data.data.map((elem) => { return { id: elem.id, name: elem.title } }) });
