...
const ThemeOptionAllTabs = ({ activeTab, values, setFieldValue, errors, touched }) => {
    const { data: categoryData, isLoading } = useQuery({queryKey: [Category], queryFn: () => request({ url: Category, params: { status: 1 } }), refetchOnWindowFocus: false, select: (data) => data.data.data });
